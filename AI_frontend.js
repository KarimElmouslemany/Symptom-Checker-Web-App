...
let chat_history = [];
function toggleChat() {
  // function for toggling the chatbot
...
const chat_button = document
  .getElementById("chat_send")
  .addEventListener("click", function (event) {
    // checks if the send button has been clicked
    const users_input = document.getElementById("chat_input").value; // gets the users input
    const remote_welcome_message = (document.getElementById(
      "chat_welcome",
    ).hidden = true); // hides the welcome message
    sendMessage(users_input); // adds the users input
  });
function sendMessage(users_input) {
  // check if users has entered a message
  if (users_input.trim() == "") {
...
function addMessage_bot(bots_reply) {
  ...
  if (bots_reply == "Bot is typing..." && checker_for_replying) {
    // checks if a certain message is being sent
...
async function botsend(user_message) {
  try {
    const response = await fetch("/chat", {
      ...
      body: JSON.stringify({ user_message, chat_history }), // converts the message to a json file
    });
