document.addEventListener("DOMContentLoaded", async() => {
  ...
  const BASE_URL = "https://int.api.service.nhs.uk/nhs-website-content/symptoms"; // url for where we are getting everything from 
  ...
  for(const letter of letters){
    const response = await fetch(`https://corsproxy.io/?https://int.api.service.nhs.uk/nhs-website-content/symptoms/?category=${letter}`,{headers:{"apikey":API_KEY}});
    ...
    console.log(data.significantLink)
    for(const symptom of NHS_symptoms ){
     ...
     await new Promise(resolve => setTimeout(resolve, 2000)); // waits a sold 1 seconds before it sends a api request for every letter. 
  }
